...
**Retail Sales ETL Pipeline** [[GitHub](https://github.com/jarviscanada/jarvis_data_eng_RuthvikPenumatcha/tree/master/databricks)]: This project was developed to create an end-to-end ETL pipeline using Databricks. The client provided 4 years of historical sales for their business as CSV files, which were ingested into a Unity Catalog Volume. The medallion architecture was followed to ingest the raw data into bronze tables and enforce the schema. Silver tables were then created to clean, augment, and apply transformations to the data. Following this, gold tables were made by aggregating the data from silver tables. The gold tables were used to power the SQL dashboard in Databricks, which displayed the trends and patterns present in the data. Finally, a CI/CD pipeline was built using Databricks CLI and Databricks Asset Bundles (DABs).
