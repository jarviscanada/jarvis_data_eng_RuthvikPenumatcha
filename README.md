...
**Competent:** Databricks, Spark, Hive, Hadoop, Kubernetes, Docker, React, Jenkins, Microsoft Azure, Google Cloud Platform (GCP), Next.js, Scikit-learn/Matplotlib
...
**Retail Sales ETL Pipeline** [[GitHub](https://github.com/jarviscanada/jarvis_data_eng_RuthvikPenumatcha/tree/master/databricks/retail_sales_etl)]: This project focuses on building an end-to-end ETL pipeline using Databricks. The client provided 4 years' worth of historical sales data in CSV format, which was ingested into a Unity Catalog Volume. The pipeline followed the medallion architecture to ingest, organize, and transform the data. Bronze tables were created to ingest the raw data with the schema applied. Silver tables were designed to clean, augment, and apply transformations to the data. Gold tables were made by aggregating the data from silver tables. The gold tables were used to power the SQL dashboard in Databricks, providing valuable insights into trends and patterns present in the data. Additionally, a CI/CD pipeline was implemented using Databricks CLI and Databricks Asset Bundles (DABs) to ensure smooth deployment and version control.
